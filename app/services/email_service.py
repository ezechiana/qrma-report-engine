    ...
    *,
    ...
    if not smtp_host:
        if app_env == "production":
            raise RuntimeError(
                "SMTP_HOST is not configured. Email cannot be sent in production."
            )
...
def send_verification_email(
    *,
    ...
    result = send_email(
        to_email=to_email,
        subject="Verify your go360 account",
        html=html,
        text=text,
    )
...
def send_password_reset_email(
    *,
    ...
    result = send_email(
        to_email=to_email,
        subject="Reset your go360 password",
        html=html,
        text=text,
    )
