...
def _get_report_type(report: ReportVersion) -> str:
    """Return the effective report type.

    Older records can have report_versions.report_type defaulted to
    'assessment' even when the JSON payload / trend payload identifies the
    report as a trend. Treat those stronger trend signals as authoritative so
    dashboard/report filters do not lump trends into assessments.
    """
    report_json = report.report_json or {}
    json_type = report_json.get("report_type") if isinstance(report_json, dict) else None
    if json_type == "trend":
        return "trend"
    ...
    column_type = getattr(report, "report_type", None)
    return column_type or json_type or "assessment"
...
def list_reports(include_archived: bool = False, report_type: str | None = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Use effective report type so legacy trend records whose column still says
    # 'assessment' are not hidden/misclassified.
    if report_type and report_type != "all":
        reports = [report for report in reports if _get_report_type(report) == report_type]
